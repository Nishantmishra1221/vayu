    ...
    <>
      <Source id="src-roads" type="geojson" data={roadsFC}>
        <Layer
          id="src-roads-line"
          type="line"
          paint={{ 'line-color': '#6C58C9', 'line-width': 2, 'line-opacity': 0.5 }}
        />
      </Source>
      <Source id="src-industrial" type="geojson" data={industrialFC}>
        <Layer
          ...
          type="fill"
          paint={{ 'fill-color': '#C25518', 'fill-opacity': 0.28 }}
        />
        <Layer
          id="src-industrial-line"
          type="line"
          paint={{ 'line-color': '#C25518', 'line-width': 1.5, 'line-dasharray': [2, 1] }}
        />
      </Source>
      ...
      {snapshot.fires.map((f, i) => (
        <Marker key={`f${i}`} longitude={f.lon} latitude={f.lat} anchor="center">
          ...
          >
            <Flame size={Math.min(22, 10 + f.frp / 3)} fill="#D63A2F" strokeWidth={1} />
          </div>
        </Marker>
      ))}
    </>
